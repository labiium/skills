...
use skillsrs_registry::{Registry, ServerHealth, ServerInfo};
...
mod tests {
    ...
    async fn test_refresh_tools() {
        ...
        // This will fail because example.com is not an MCP server
        let result = manager.add_upstream(config).await;
        assert!(
            result.is_err(),
            "Expected connection to fail with non-MCP URL"
        );
    }
}
