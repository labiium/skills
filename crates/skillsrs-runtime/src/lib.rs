...
use skillsrs_core::{BundledTool, CallableId, CallableKind, ToolResult, ToolResultContent};
...
impl Runtime {
    ...
    async fn execute_tool(
        &self,
        ctx: &ExecContext,
        record: &skillsrs_core::CallableRecord,
    ) -> Result<ToolResult> {
                        ...
                        match item_type {
                            ...
                            "resource" => {
                                ...
                                Some(ToolResultContent::Resource {
                                    resource: skillsrs_core::ResourceContent {
                                        uri: uri.to_string(),
...
mod tests {
    use super::*;
    use skillsrs_core::{CostHints, RiskTier, SchemaDigest};
    use skillsrs_registry::Registry;

    fn make_test_tool() -> skillsrs_core::CallableRecord {
        ...
        let id = CallableId::tool("test-server", "test-tool", digest.as_str());

        skillsrs_core::CallableRecord {
            id: id.clone(),
    ...
    async fn test_execute_tool() {
        ...
        // Without a real upstream server connection, this will fail with UpstreamError
        let result = runtime.execute(ctx).await;
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            RuntimeError::UpstreamError(_)
        ));
    }
    ...
    async fn test_validation_missing_required() {
        ...
        let result = runtime.execute(ctx).await;
        assert!(result.is_err());
        assert!(matches!(result, Err(RuntimeError::ValidationFailed(_))));
    }
    ...
    async fn test_callable_not_found() {
        ...
        let result = runtime.execute(ctx).await;
        assert!(result.is_err());
        assert!(matches!(result, Err(RuntimeError::CallableNotFound(_))));
    }
}
