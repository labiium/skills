...
"#;
    ...
    fn test_validate_name() {
        assert!(AgentSkill::validate_name("valid-name").is_ok());
        assert!(AgentSkill::validate_name("name-123").is_ok());
        assert!(AgentSkill::validate_name("a").is_ok());

        assert!(AgentSkill::validate_name("Invalid-Name").is_err()); // uppercase
        assert!(AgentSkill::validate_name("-invalid").is_err()); // starts with hyphen
        assert!(AgentSkill::validate_name("invalid-").is_err()); // ends with hyphen
        assert!(AgentSkill::validate_name("invalid--name").is_err()); // consecutive hyphens
        assert!(AgentSkill::validate_name("").is_err()); // empty
    }
...
"#;
        ...
        let tools = skill.parse_allowed_tools();
