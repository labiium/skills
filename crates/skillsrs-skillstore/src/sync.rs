...
use crate::agent_skills::AgentSkill;
use crate::Result;
...
impl AgentSkillsSync {
    ...
    async fn sync_repo(
        ...
        // Allow file:// for testing
        {
            ...
            return Err(crate::SkillStoreError::InvalidManifest(format!(
                "Invalid repo format: {}",
                config.repo
            )));
        };
        ...
        if !output.status.success() {
            return Err(crate::SkillStoreError::Io(std::io::Error::other(format!(
                "Git clone failed: {}",
        ...
        for skill_path in &skills_to_import {
            let skill_name = skill_path
                ...
                .and_then(|n| n.to_str())
                .ok_or_else(|| {
                    crate::SkillStoreError::InvalidManifest("Invalid skill name".to_string())
                })?;
    ...
    async fn get_commit_sha(&self, repo_path: &Path) -> Result<String> {
        ...
        if !output.status.success() {
            return Err(crate::SkillStoreError::Io(std::io::Error::other(
