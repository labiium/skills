...
use crate::{CallableId, CallableKind, CallableRecord, CostHints, RiskTier, SchemaDigest};
...
impl PersistenceLayer {
    ...
    fn row_to_callable(&self, row: sqlx::sqlite::SqliteRow) -> Result<CallableRecord> {
        ...
        let risk_tier: RiskTier = risk_tier_str
            .parse()
            .map_err(|e: crate::CoreError| PersistenceError::InvalidData(e.to_string()))?;
...
mod tests {
    ...
    async fn test_execution_history() {
        ...
        let callable_id = CallableId::from("test://tool@1.0");
        ...
        let history = persistence
            .get_execution_history(&callable_id, 10)
            .await
            .unwrap();
